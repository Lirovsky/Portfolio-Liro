...
export default function Home() {
  const links = [
    ...
    {
      id: 3,
      link: "https://drive.google.com/file/d/10C8bYC4vNn83R-vDSkqxt2JPi-h0ECfQ/view?usp=drive_link",
      target: "_blank",
      icon: <FaFileAlt />,
    },
  ];
          ...
          <div className="mt-5 flex sm:mt-0">
            {links.map((item) => (
              <a
                ...
                key={item.id}
                target={item.target}
                className="mx-4 text-4xl text-yellow-200 transition-all duration-500 hover:-translate-y-4 sm:mx-0 sm:ms-7 md:ms-9 md:text-5xl lg:ms-12 lg:text-7xl dark:text-emerald-500"
              >
                {item.icon}
              </a>
            ))}
